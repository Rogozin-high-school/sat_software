...
#define verbose(format, ...) log(WHITE, MODULE_NAME, format __VA_OPT__(, ) __VA_ARGS__)
#define debug(format, ...) log(LIGHT_GREY, MODULE_NAME, format __VA_OPT__(, ) __VA_ARGS__)
#define info(format, ...) log(CYAN, MODULE_NAME, format __VA_OPT__(, ) __VA_ARGS__)
...
#define log_throw_exception(ex) \
      err(ITALIC CYAN "%s" RESET RED " has thrown an exception!\n" PADDING BOLD "%s: " RESET RED "%s", FUNCTION_NAME, demangle(typeid(ex).name()).c_str(), ex.what())
#define log_catch_and_throw_exception() \
...
{
      log(WHITE, moduleName, ITALIC CYAN "%s", funcName); // verbose
